-- New function: update fees by composite key (payment_brand + payment_method type)
CREATE OR REPLACE FUNCTION public.update_ordertotals_bank_fees_by_pair(brand_name text, payment_type text, batch_size integer DEFAULT 5000)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  LOOP
    WITH batch AS (
      ...
      WHERE lower(o.payment_brand) = lower(brand_name)
        AND lower(o.payment_method) = lower(payment_type)
