-- Update transactions_summary to support company filter
CREATE OR REPLACE FUNCTION public.transactions_summary(date_from date, date_to date, p_brand_name text DEFAULT NULL::text, p_company text DEFAULT NULL::text)
...
-- Update get_cost_of_sales to support company filter
CREATE OR REPLACE FUNCTION public.get_cost_of_sales(date_from date, date_to date, p_brand_name text DEFAULT NULL::text, p_company text DEFAULT NULL::text)
 RETURNS numeric
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- Update get_epayment_charges to support company filter
CREATE OR REPLACE FUNCTION public.get_epayment_charges(date_from date, date_to date, p_brand_name text DEFAULT NULL::text, p_company text DEFAULT NULL::text)
 RETURNS numeric
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $$
  ...
  WHERE o.created_at_date::date BETWEEN date_from AND date_to
    ...
    AND (p_company IS NULL OR o.company = p_company);
$$;

-- Update get_points_summary to support company filter
CREATE OR REPLACE FUNCTION public.get_points_summary(date_from date, date_to date, p_brand_name text DEFAULT NULL::text, p_company text DEFAULT NULL::text)
