...
Deno.serve(async (req) => {
  ...
  try {
    ...
    requestBody = body;
    console.log('Received supplier data:', JSON.stringify(body));
    ...
    // Upsert to suppliers table based on mode
    ...
    if (upsertError) {
      console.error(`Error upserting to ${tables.suppliers}:`, upsertError);
      responseStatus = 400;
    ...
    console.log(`Successfully upserted to ${tables.suppliers}:`, resultData);
    responseMessage = `Supplier saved to ${tables.suppliers} table (${apiMode} mode)`;
    await logApiCall();
