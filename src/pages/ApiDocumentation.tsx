...
const API_ENDPOINTS = [
  ...
  {
    id: "brand-balance",
    name: "Brand Coins Balance (Report & Alert)",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-brand-balance`,
    method: "POST",
    description: "Report a brand's current coins balance. The balance is saved as a snapshot in brand_coin_balances. If the reported balance is less than or equal to the brand's Reorder Point, a low-stock alert (email + in-app + push notification) is automatically sent to the Coins Purchase department members and admins. Requires an API key with Brand permission (allow_brand).",
    upsertKey: null,
    fields: [
    ...
    responseFields: [
      { name: "success", type: "Boolean", note: "true if balance saved" },
      { name: "record_id", type: "UUID", note: "ID of the saved balance snapshot" },
      { name: "brand_id", type: "UUID", note: "Resolved brand ID" },
      { name: "brand_name", type: "Text", note: "Brand name" },
      { name: "brand_code", type: "Text", note: "Brand code" },
      { name: "balance", type: "Decimal", note: "Echoed balance" },
  ...
  {
    id: "brand-abc-a-stock",
    name: "Brand ABC Class A Stock Status",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-brand-abc-a-stock`,
    method: "GET",
    description: "Returns inventory metrics (Reorder Point, Average Consumption Per Day, Safety Stock, and Current Balance from the latest closed shift) for all active brands classified as ABC Class A. Requires an API key with Brand permission (allow_brand).",
    fields: [],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if request succeeded" },
      { name: "count", type: "Integer", note: "Number of ABC Class A brands returned" },
      { name: "data[].brand_id", type: "UUID", note: "Brand unique identifier" },
      { name: "data[].brand_name", type: "Text", note: "Brand name" },
      { name: "data[].brand_code", type: "Text", note: "Brand code" },
      { name: "data[].supplier_hub_code", type: "Text", note: "Supplier hub code" },
      { name: "data[].abc_class", type: "Text", note: "Always 'A'" },
      { name: "data[].reorder_point", type: "Decimal", note: "Reorder point threshold" },
      { name: "data[].average_consumption_per_day", type: "Decimal", note: "Average daily coin consumption" },
      { name: "data[].safety_stock", type: "Decimal", note: "Safety stock level" },
      { name: "data[].current_balance", type: "Decimal", note: "Closing balance from the latest closed shift (null if none)" },
      { name: "data[].last_shift_closed_at", type: "DateTime", note: "When that shift was closed (null if none)" },
    ],
  },
  {
    id: "products-list",
  ...
  {
    id: "crm-customer-info",
    name: "CRM Customer Info",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-crm-customer-info`,
    method: "POST",
    description: "Send a customer phone number and get back the customer profile (name, email, creation date, last transaction date/time), total value of the last 10 orders, total transaction value for the last 2 months, and an array of the last 10 orders with their product, brand, quantity and totals. Requires an API key with CRM permission (allow_crm). Also supports GET with ?customer_phone={PHONE}.",
    upsertKey: null,
    fields: [
      { name: "customer_phone", type: "Text", required: true, note: "Customer phone number (also accepted as 'phone' or query param on GET)" },
    ],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if customer found" },
      { name: "customer.customer_phone", type: "Text", note: "Echoed phone number" },
      { name: "customer.customer_name", type: "Text", note: "Customer name" },
      { name: "customer.email", type: "Text", note: "Customer email" },
      { name: "customer.creation_date", type: "DateTime", note: "Customer registration date" },
      { name: "customer.last_transaction_date", type: "DateTime", note: "Date & time of the latest transaction" },
      { name: "customer.status", type: "Text", note: "Customer status (active / suspended)" },
      { name: "customer.is_blocked", type: "Boolean", note: "true if the customer is blocked" },
      { name: "customer.customer_group", type: "Text", note: "Customer group" },
      { name: "customer.total_orders_count", type: "Integer", note: "Total number of orders on record" },
      { name: "customer.total_value_last_orders", type: "Decimal", note: "Total value of the returned last 10 orders" },
      { name: "customer.total_value_last_2_months", type: "Decimal", note: "Total transaction value in the last 2 months" },
      { name: "orders_count", type: "Integer", note: "Number of orders returned (max 10)" },
      { name: "orders[].order_number", type: "Text", note: "Order number" },
      { name: "orders[].order_date", type: "DateTime", note: "Order date & time" },
      { name: "orders[].payment_method", type: "Text", note: "Payment method" },
      { name: "orders[].payment_brand", type: "Text", note: "Payment brand" },
      { name: "orders[].payment_card_number", type: "Text", note: "Masked payment card number" },
      { name: "orders[].payment_reference", type: "Text", note: "Payment reference" },
      { name: "orders[].customer_ip", type: "Text", note: "Customer IP address used for the order" },
      { name: "orders[].customer_ip_country", type: "Text", note: "Country resolved from the customer IP" },
      { name: "orders[].device_fingerprint", type: "Text", note: "Device fingerprint" },
      { name: "orders[].profit_center", type: "Text", note: "Profit center" },
      { name: "orders[].status_description", type: "Text", note: "Status description" },
      { name: "orders[].register_user_id", type: "Text", note: "Registering user ID" },
      { name: "orders[].player_id", type: "Text", note: "Player ID" },
      { name: "orders[].order_status", type: "Text", note: "Order status" },
      { name: "orders[].total_qty", type: "Decimal", note: "Total quantity of the order" },
      { name: "orders[].order_total", type: "Decimal", note: "Order total value" },
      { name: "orders[].lines[].product_name", type: "Text", note: "Product name" },
      { name: "orders[].lines[].brand_name", type: "Text", note: "Brand name" },
      { name: "orders[].lines[].brand_code", type: "Text", note: "Brand code" },
      { name: "orders[].lines[].qty", type: "Decimal", note: "Line quantity" },
      { name: "orders[].lines[].coins_number", type: "Decimal", note: "Coins quantity (if any)" },
      { name: "orders[].lines[].unit_price", type: "Decimal", note: "Unit price" },
      { name: "orders[].lines[].total", type: "Decimal", note: "Line total" },
    ],
  },
  {
    ...
    id: "crm-login",
    name: "CRM Login",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-crm-login`,
    method: "POST",
    description: "Authenticate a user for external CRM application. Returns a session ID (access_token) that can be used for subsequent API calls and WebView authentication. No API key required - uses email/password authentication.",
    fields: [
      { name: "email", type: "Text", required: true, note: "User email address" },
      { name: "password", type: "Text", required: true, note: "User password" },
    ],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if login successful" },
      { name: "session_id", type: "Text", note: "Access token for API calls (Bearer token)" },
      { name: "refresh_token", type: "Text", note: "Token to refresh expired session" },
      { name: "expires_at", type: "Integer", note: "Session expiry timestamp (Unix)" },
      { name: "user.id", type: "UUID", note: "User unique identifier" },
      { name: "user.email", type: "Text", note: "User email" },
      { name: "user.user_name", type: "Text", note: "Display name" },
      { name: "user.roles", type: "Array", note: "User roles (admin, user, etc.)" },
      { name: "user.admin_access", type: "Boolean", note: "true if user has 'admin' role" },
      { name: "user.shift_id", type: "UUID", note: "Assigned shift id (null if none)" },
      { name: "user.shift_name", type: "Text", note: "Assigned shift name (null if none)" },
      { name: "user.permissions", type: "Array", note: "User access permissions" },
    ],
  },
  {
    id: "crm-shift-check",
    name: "CRM Shift Check",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-crm-shift-check`,
    method: "POST",
    description: "Check if the authenticated user has a scheduled shift at the current time (±5 minutes buffer). Returns shift details and status (open/pending/closed/no_assignment). Requires Bearer token from CRM Login.",
    fields: [
      { name: "Authorization", type: "Header", required: true, note: "Bearer {session_id} from CRM Login" },
    ],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if check completed" },
      { name: "has_shift", type: "Boolean", note: "true if shift found in time window" },
      { name: "shift_status", type: "Text", note: "open / pending / closed / no_assignment / outside_window" },
      { name: "shift_session_id", type: "Text", note: "Session ID if shift is open" },
      { name: "shift_assignment_id", type: "Text", note: "Assignment ID if shift is pending" },
      { name: "shift_name", type: "Text", note: "Shift name" },
      { name: "shift_start_time", type: "Text", note: "Shift start time (HH:MM:SS)" },
      { name: "shift_end_time", type: "Text", note: "Shift end time (HH:MM:SS)" },
      { name: "shift_type", type: "Text", note: "Shift type (e.g., support, sales)" },
      { name: "assignment_date", type: "Date", note: "Assignment date (YYYY-MM-DD)" },
    ],
  },
  {
    id: "crm-shift-checkin",
    name: "CRM Shift Check-in",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-crm-shift-checkin`,
    method: "POST",
    description: "Check in to a shift - opens a shift session with the first order numbers for Purple and Salla. Automatically records attendance. Returns the shift reference (session ID). Requires Bearer token from CRM Login.",
    fields: [
      { name: "Authorization", type: "Header", required: true, note: "Bearer {session_id} from CRM Login" },
      { name: "first_order_number", type: "Text", required: true, note: "First Purple order number for the shift" },
      { name: "salla_first_order_number", type: "Text", required: false, note: "First Salla order number (optional)" },
    ],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if check-in successful" },
      { name: "shift_session_id", type: "Text", note: "Shift session UUID" },
      { name: "shift_reference", type: "Text", note: "Shift reference (same as session ID)" },
      { name: "shift_assignment_id", type: "Text", note: "Assignment ID" },
      { name: "shift_name", type: "Text", note: "Shift name" },
      { name: "shift_start_time", type: "Text", note: "Shift start time" },
      { name: "shift_end_time", type: "Text", note: "Shift end time" },
      { name: "shift_type", type: "Text", note: "Shift type" },
      { name: "assignment_date", type: "Date", note: "Assignment date" },
      { name: "first_order_number", type: "Text", note: "Purple first order number" },
      { name: "salla_first_order_number", type: "Text", note: "Salla first order number" },
      { name: "opened_at", type: "DateTime", note: "Session open timestamp" },
    ],
  },
  {
    id: "bank-accounts",
    name: "Bank Accounts (GET Active)",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/external-bank-treasury-api/bank-accounts`,
    method: "GET",
    description: "Returns the list of all active bank accounts (banks marked is_active = true). Requires an API key with Banks & Treasury permission (allow_bank_treasury).",
    fields: [],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if request succeeded" },
      { name: "count", type: "Integer", note: "Number of active bank accounts returned" },
      { name: "data[].id", type: "UUID", note: "Bank account unique identifier" },
      { name: "data[].bank_code", type: "Text", note: "Bank code" },
      { name: "data[].bank_name", type: "Text", note: "Bank name (English)" },
      { name: "data[].bank_name_ar", type: "Text", note: "Bank name (Arabic)" },
      { name: "data[].account_number", type: "Text", note: "Account number" },
      { name: "data[].iban", type: "Text", note: "IBAN" },
      { name: "data[].swift_code", type: "Text", note: "SWIFT code" },
      { name: "data[].branch_name", type: "Text", note: "Branch name" },
      { name: "data[].currency_id", type: "UUID", note: "Currency reference" },
      { name: "data[].opening_balance", type: "Decimal", note: "Opening balance" },
      { name: "data[].current_balance", type: "Decimal", note: "Current balance" },
      { name: "data[].allow_negative_balance", type: "Boolean", note: "Allows negative balance (credit cards)" },
      { name: "data[].is_active", type: "Boolean", note: "Always true on this endpoint" },
    ],
  },
  {
    id: "treasuries",
    name: "Treasuries (GET Active)",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/external-bank-treasury-api/treasuries`,
    method: "GET",
    description: "Returns the list of all active treasuries (treasuries marked is_active = true). Requires an API key with Banks & Treasury permission (allow_bank_treasury).",
    fields: [],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if request succeeded" },
      { name: "count", type: "Integer", note: "Number of active treasuries returned" },
      { name: "data[].id", type: "UUID", note: "Treasury unique identifier" },
      { name: "data[].treasury_code", type: "Text", note: "Treasury code" },
      { name: "data[].treasury_name", type: "Text", note: "Treasury name (English)" },
      { name: "data[].treasury_name_ar", type: "Text", note: "Treasury name (Arabic)" },
      { name: "data[].responsible_user_id", type: "UUID", note: "Responsible user" },
      { name: "data[].department_id", type: "UUID", note: "Department reference" },
      { name: "data[].currency_id", type: "UUID", note: "Currency reference" },
      { name: "data[].opening_balance", type: "Decimal", note: "Opening balance" },
      { name: "data[].current_balance", type: "Decimal", note: "Current balance" },
      { name: "data[].max_balance", type: "Decimal", note: "Maximum allowed balance" },
      { name: "data[].is_active", type: "Boolean", note: "Always true on this endpoint" },
      { name: "data[].notes", type: "Text", note: "Notes" },
    ],
  },
  {
    id: "purple-transaction",
    name: "Purple Transaction (POST by Date Range)",
    endpoint: `${SUPABASE_FUNCTIONS_URL}/api-purple-transaction`,
    method: "POST",
    description: "Returns Purple transaction rows from purpletransaction for a given date range (inclusive). Requires an API key with Purple Transaction permission (allow_purple_transaction). Filters out soft-deleted rows.",
    fields: [
      { name: "from_date", type: "Date (YYYY-MM-DD)", required: true, note: "Start date (inclusive), based on created_at_date_int" },
      { name: "to_date", type: "Date (YYYY-MM-DD)", required: true, note: "End date (inclusive)" },
      { name: "company", type: "Text", required: false, note: "Optional company filter (e.g. Purple, Asus)" },
      { name: "payment_method", type: "Text", required: false, note: "Optional payment method filter (e.g. hyperpay, salla)" },
      { name: "limit", type: "Integer", required: false, note: "Maximum rows to return (default 100000, max 100000)" },
    ],
    responseFields: [
      { name: "success", type: "Boolean", note: "true if request succeeded" },
      { name: "from_date", type: "Text", note: "Echo of requested from_date" },
      { name: "to_date", type: "Text", note: "Echo of requested to_date" },
      { name: "count", type: "Integer", note: "Number of rows returned" },
      { name: "data[].id", type: "UUID", note: "Transaction unique identifier" },
      { name: "data[].order_number", type: "Text", note: "Order number (per line)" },
      { name: "data[].ordernumber", type: "Text", note: "Original order number for grouping" },
      { name: "data[].line_no", type: "Integer", note: "Line number within order" },
      { name: "data[].created_at_date", type: "DateTime", note: "Transaction date (KSA)" },
      { name: "data[].created_at_date_int", type: "Integer", note: "Transaction date as YYYYMMDD" },
      { name: "data[].company", type: "Text", note: "Company (Purple / Asus)" },
      { name: "data[].brand_name", type: "Text", note: "Brand name" },
      { name: "data[].brand_code", type: "Text", note: "Brand code" },
      { name: "data[].product_name", type: "Text", note: "Product name" },
      { name: "data[].product_id", type: "Text", note: "Product identifier" },
      { name: "data[].coins_number", type: "Decimal", note: "Coins quantity" },
      { name: "data[].qty", type: "Decimal", note: "Quantity" },
      { name: "data[].unit_price", type: "Decimal", note: "Unit price" },
      { name: "data[].cost_price", type: "Decimal", note: "Cost price" },
      { name: "data[].cost_sold", type: "Decimal", note: "Cost of sold quantity" },
      { name: "data[].total", type: "Decimal", note: "Line total" },
      { name: "data[].profit", type: "Decimal", note: "Line profit" },
      { name: "data[].bank_fee", type: "Decimal", note: "Bank fee" },
      { name: "data[].customer_name", type: "Text", note: "Customer name" },
      { name: "data[].customer_phone", type: "Text", note: "Customer phone" },
      { name: "data[].payment_method", type: "Text", note: "Payment method" },
      { name: "data[].payment_type", type: "Text", note: "Payment type" },
      { name: "data[].payment_brand", type: "Text", note: "Payment brand" },
      { name: "data[].payment_term", type: "Text", note: "Payment term" },
      { name: "data[].payment_reference", type: "Text", note: "Payment reference" },
      { name: "data[].user_name", type: "Text", note: "Sales person / user" },
      { name: "data[].sales_person", type: "Text", note: "Sales person" },
      { name: "data[].vendor_name", type: "Text", note: "Vendor name" },
      { name: "data[].order_status", type: "Text", note: "Order status" },
      { name: "data[].status", type: "Integer", note: "Status code" },
      { name: "data[].status_description", type: "Text", note: "Status description" },
      { name: "data[].transaction_type", type: "Text", note: "Transaction type" },
      { name: "data[].trans_type", type: "Text", note: "Trans type" },
      { name: "data[].profit_center", type: "Text", note: "Profit center" },
      { name: "data[].revenue_source", type: "Text", note: "Revenue source (Purple/Salla/Asus)" },
      { name: "data[].media", type: "Text", note: "Media channel" },
      { name: "data[].is_point", type: "Boolean", note: "Is points payment" },
      { name: "data[].point_value", type: "Decimal", note: "Points value" },
      { name: "data[].player_id", type: "Text", note: "Player ID" },
      { name: "data[].sendodoo", type: "Boolean", note: "Synced to Odoo" },
      { name: "data[].source", type: "Text", note: "Origin source (EXCEL / API)" },
      { name: "data[].created_at", type: "DateTime", note: "Record creation timestamp" },
      { name: "data[].updated_at", type: "DateTime", note: "Last update timestamp" },
    ],
  },
];
