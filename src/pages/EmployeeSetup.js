...
export default function EmployeeSetup() {
    ...
    const [usersWithoutEmployee, setUsersWithoutEmployee] = useState([]);
    ...
    const [filterPayrollCountry, setFilterPayrollCountry] = useState("all");
    const [filterLetter, setFilterLetter] = useState("");
    const [dialogOpen, setDialogOpen] = useState(false);
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const [payrollDialogOpen, setPayrollDialogOpen] = useState(false);
    const [payrollDialogEmp, setPayrollDialogEmp] = useState(null);
    const [payrollDialogRows, setPayrollDialogRows] = useState([]);
    const [payrollDialogLoading, setPayrollDialogLoading] = useState(false);
    const openPayrollDialog = async (emp) => {
    ...
    const [employeeVacationBalances, setEmployeeVacationBalances] = useState([]);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [employeesRes, departmentsRes, positionsRes, profilesRes, vacationCodesRes, insurancePlansRes, shiftPlansRes, docTypesRes, attendanceTypesRes, businessUnitsRes, costCentersRes, buCcMappingRes,] = await Promise.all([
                supabase
                    .from("employees")
                    .select(`
            *,
          ...
          `)
            ...
            const sortedEmployees = (employeesRes.data || []).sort((a, b) => a.first_name.localeCompare(b.first_name, 'en'));
            ...
            setBuCcMappings(buCcMappingRes.data || []);
    ...
    const toggleUserSelection = (userId) => {
        setSelectedUsersToAdd(prev => prev.includes(userId)
    ...
    const applyFilters = (term, department, job, letter, searchMode, businessUnit = filterBusinessUnit, payrollCountry = filterPayrollCountry) => {
        ...
        if (term.trim()) {
            ...
            filtered = filtered.filter((emp) => {
                ...
                return (emp.employee_number.toLowerCase().includes(t) ||
        ...
        if (businessUnit && businessUnit !== "all") {
            filtered = filtered.filter((emp) => emp.working_business_unit_id === businessUnit);
        }
    ...
    const handleSearchByChange = (mode) => {
        setSearchBy(mode);
    ...
    const handleDocumentUpload = async (event) => {
        const file = event.target.files?.[0];
        if (!file || !selectedEmployee || !documentFormData.document_type_id)
            return;
    ...
    const openEditDialog = (employee) => {
        ...
        setFormData({
            ...
            dopay_employee_number: employee.dopay_employee_number || "",
            ...
            working_business_unit_id: employee.working_business_unit_id || "",
    ...
    const fetchEmployeeVacationTypes = async (employeeId) => {
        try {
            ...
            const vacationTypes = (data || []).map(d => ({
                ...
                vacation_code: d.vacation_codes,
                balance: d.balance || 0,
    ...
    const openDocumentDialog = (employee) => {
        setSelectedEmployee(employee);
        fetchEmployeeDocuments(employee.id);
        setDocumentDialogOpen(true);
    };
    const openContactDialog = (employee) => {
        setSelectedEmployee(employee);
        fetchEmployeeContacts(employee.id);
    ...
    const handleSave = async () => {
        ...
        if (!formData.first_name)
            missingFields.push(language === "ar" ? "الاسم الأول" : "First Name");
        if (!formData.last_name)
            missingFields.push(language === "ar" ? "اسم العائلة" : "Last Name");
        if (!formData.job_start_date)
            missingFields.push(language === "ar" ? "تاريخ بداية العمل" : "Job Start Date");
        if (missingFields.length > 0) {
            toast.error((language === "ar" ? "يرجى ملء الحقول المطلوبة: " : "Please fill required fields: ") + missingFields.join(", "));
            return;
        }
        ...
        try {
            ...
            if (employeeVacationBalances.length > 0) {
                ...
                if (vacError)
                    console.error("Error saving vacation types:", vacError);
            }
            setDialogOpen(false);
            fetchData();
        }
    ...
    return (<div className="container mx-auto p-6 space-y-6">
        ...
        <CardContent>
          <div className="space-y-4 mb-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="relative flex-1 min-w-[200px] max-w-sm">
                ...
                <Input placeholder={language === "ar"
            ? searchBy === "zk"
                ? "بحث برمز الحضور..."
                : "بحث..."
            : searchBy === "zk"
                ? "Search by ZK code..."
                : "Search..."} value={searchTerm} onChange={(e) => handleSearch(e.target.value)} className="pl-10"/>
              </div>

              {/* Search Mode */}
              <Select value={searchBy} onValueChange={(v) => handleSearchByChange(v)}>
              ...
              <Select value={filterJob} onValueChange={handleJobFilter}>
                ...
                <SelectContent>
                  ...
                  {jobPositions.map((job) => (<SelectItem key={job.id} value={job.id}>
              ...
              <Select value={filterBusinessUnit} onValueChange={handleBusinessUnitFilter}>
                ...
                <SelectContent>
                  ...
                  {businessUnits.map((bu) => (<SelectItem key={bu.id} value={bu.id}>
              ...
              {/* Clear Filters */}
              {(filterDepartment !== "all" || filterJob !== "all" || filterBusinessUnit !== "all" || filterPayrollCountry !== "all" || filterLetter || searchTerm) && (<Button variant="ghost" size="sm" onClick={() => {
                setSearchTerm("");
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex flex-wrap gap-1">
                {(language === "ar" ? arabicLetters : englishLetters).map((letter) => (<Button key={letter} variant={filterLetter === letter ? "default" : "outline"} size="sm" className="w-8 h-8 p-0 text-xs font-medium" onClick={() => handleLetterFilter(letter)}>
                    {letter}
                  </Button>))}
              </div>
          ...
          {viewMode === "table" ? (<div className="rounded-md border">
              <Table>
                ...
                <TableBody>
                  {loading ? (<TableRow>
                    ...
                    </TableRow>) : (employees.map((emp) => (<TableRow key={emp.id}>
                        <TableCell>
                          <Avatar>
                    ...
                    : (emp.job_positions?.position_name || "-")}
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <Button variant="ghost" size="icon" onClick={() => navigate(`/employee-profile/${emp.id}`)} title={language === "ar" ? "عرض" : "View"}>
                              <Eye className="h-4 w-4"/>
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => openDocumentDialog(emp)} title={language === "ar" ? "المستندات" : "Documents"}>
                              <FileText className="h-4 w-4"/>
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => openEditDialog(emp)}>
                              <Edit className="h-4 w-4"/>
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => openPayrollDialog(emp)} title={language === "ar" ? "عناصر الرواتب" : "Payroll Elements"}>
              ...
              {loading ? (<div className="col-span-full text-center py-8">
                  {language === "ar" ? "جاري التحميل..." : "Loading..."}
                </div>) : employees.length === 0 ? (<div className="col-span-full text-center py-8">
                  {language === "ar" ? "لا توجد بيانات" : "No data found"}
                </div>) : (employees.map((emp) => {
                const empDrAccount = buCcMappings.find((x) => x.cost_center_id === emp.cost_center_id &&
                    (!emp.working_business_unit_id || x.business_unit_id === emp.working_business_unit_id))?.payroll_dr_account;
                return (<Card key={emp.id} className={`hover:shadow-lg transition-shadow ${!empDrAccount ? "border-2 border-destructive" : ""}`}>
                    <CardContent className="p-4">
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="flex items-center gap-1 mt-4">
                          <Button variant="ghost" size="icon" onClick={() => navigate(`/employee-profile/${emp.id}`)}>
                            <Eye className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => openDocumentDialog(emp)}>
                            <FileText className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => openEditDialog(emp)}>
                            <Edit className="h-4 w-4"/>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <Tabs defaultValue="info">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="md:col-span-3 flex justify-center mb-4">
                  <div className="relative">
                    ...
                    <Button variant="outline" size="icon" className="absolute bottom-0 right-0 rounded-full h-8 w-8" onClick={() => photoInputRef.current?.click()} disabled={uploadingPhoto}>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={formData.employee_number} onChange={(e) => setFormData({ ...formData, employee_number: e.target.value })} placeholder={language === "ar" ? "سيتم إنشاؤه تلقائياً" : "Auto-generated on save"} disabled={!selectedEmployee}/>
                </div>

                <div className="space-y-2">
                  <Label>{language === "ar" ? "كود جهاز البصمة" : "ZK Attendance Code"}</Label>
                  <Input value={formData.zk_employee_code} onChange={(e) => setFormData({ ...formData, zk_employee_code: e.target.value })} placeholder={language === "ar" ? "رقم الموظف في جهاز ZK" : "Employee number in ZK machine"}/>
                </div>

                <div className="space-y-2">
                  <Label>{language === "ar" ? "رقم موظف Dopay" : "Dopay Employee Number"}</Label>
                  <Input value={formData.dopay_employee_number} onChange={(e) => setFormData({ ...formData, dopay_employee_number: e.target.value })} placeholder={language === "ar" ? "رقم الموظف في Dopay" : "Employee number in Dopay"}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={formData.first_name_ar} onChange={(e) => setFormData({ ...formData, first_name_ar: e.target.value })} dir="rtl"/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={formData.last_name_ar} onChange={(e) => setFormData({ ...formData, last_name_ar: e.target.value })} dir="rtl"/>
                </div>

                <div className="space-y-2">
                  <Label>{language === "ar" ? "ربط بمستخدم" : "Link to User"}</Label>
                  <Select value={formData.user_id || "_none_"} onValueChange={(value) => setFormData({ ...formData, user_id: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={formData.work_mobile} onChange={(e) => setFormData({ ...formData, work_mobile: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.gender || "_none_"} onValueChange={(value) => setFormData({ ...formData, gender: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.nationality || "_none_"} onValueChange={(value) => setFormData({ ...formData, nationality: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.marital_status || "_none_"} onValueChange={(value) => setFormData({ ...formData, marital_status: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                      <SelectValue placeholder={language === "ar" ? "اختر" : "Select"}/>
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.religion || "_none_"} onValueChange={(value) => setFormData({ ...formData, religion: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                      <SelectValue placeholder={language === "ar" ? "اختر" : "Select"}/>
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2 md:col-span-3">
                  ...
                  <Input value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder={language === "ar" ? "أدخل العنوان الكامل" : "Enter full address"}/>
                </div>
              </div>
            </TabsContent>
                  ...
                  <CardContent className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="space-y-2">
                        ...
                        <Select value={contactFormData.contact_type || "_none_"} onValueChange={(value) => setContactFormData({ ...contactFormData, contact_type: value === "_none_" ? "" : value })}>
                          <SelectTrigger>
                            <SelectValue placeholder={language === "ar" ? "اختر" : "Select"}/>
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input value={contactFormData.contact_name} onChange={(e) => setContactFormData({ ...contactFormData, contact_name: e.target.value })}/>
                      </div>
                      <div className="space-y-2">
                        <Label>{language === "ar" ? "رقم الهاتف" : "Phone Number"}</Label>
                        <Input value={contactFormData.contact_phone} onChange={(e) => setContactFormData({ ...contactFormData, contact_phone: e.target.value })}/>
                      </div>
                      <div className="space-y-2">
                        <Label>{language === "ar" ? "العنوان" : "Address"}</Label>
                        <Input value={contactFormData.contact_address} onChange={(e) => setContactFormData({ ...contactFormData, contact_address: e.target.value })}/>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <input type="checkbox" checked={contactFormData.is_emergency_contact} onChange={(e) => setContactFormData({ ...contactFormData, is_emergency_contact: e.target.checked })} className="h-4 w-4 rounded border-gray-300"/>
            ...
            : (language === "ar" ? "إضافة" : "Add")}
                      ...
                      {editingContact && (<Button variant="outline" size="sm" onClick={() => {
                setEditingContact(null);
                setContactFormData({
                    contact_type: "",
                    contact_name: "",
                ...
                {selectedEmployee && employeeContacts.length > 0 && (<Card>
                    <CardHeader className="py-3">
                      <CardTitle className="text-sm">
                    ...
                    <CardContent>
                      <Table>
                        ...
                        <TableBody>
                          {employeeContacts.map((contact) => (<TableRow key={contact.id}>
                              <TableCell>
                ...
                {selectedEmployee && employeeContacts.length === 0 && (<p className="text-center text-muted-foreground py-4">
            ...
            <TabsContent value="job">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.department_id || "_none_"} onValueChange={(value) => setFormData({ ...formData, department_id: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.job_position_id || "_none_"} onValueChange={(value) => setFormData({ ...formData, job_position_id: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                      <SelectValue placeholder={language === "ar" ? "اختر المسمى" : "Select Position"}/>
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="_none_">{language === "ar" ? "اختر" : "Select"}</SelectItem>
                      {jobPositions.map((pos) => (<SelectItem key={pos.id} value={pos.id}>
                          {pos.position_name}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.working_business_unit_id || "_none_"} onValueChange={(value) => {
            ...
            setFormData({ ...formData, working_business_unit_id: value === "_none_" ? "" : value });
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.cost_center_id || "_none_"} onValueChange={(value) => setFormData({ ...formData, cost_center_id: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                      <SelectValue placeholder={language === "ar" ? "اختر مركز التكلفة" : "Select Cost Center"}/>
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="_none_">{language === "ar" ? "اختر" : "Select"}</SelectItem>
                      {costCenters.map((cc) => (<SelectItem key={cc.id} value={cc.id}>
                ...
                {(() => {
            const m = buCcMappings.find((x) => x.cost_center_id === formData.cost_center_id &&
                (!formData.working_business_unit_id || x.business_unit_id === formData.working_business_unit_id));
            if (!m?.payroll_dr_account)
                return null;
            return (<div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.payroll_country || "_none_"} onValueChange={(value) => setFormData({ ...formData, payroll_country: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input type="date" value={formData.job_start_date} onChange={(e) => setFormData({ ...formData, job_start_date: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.manager_id || "_none_"} onValueChange={(value) => setFormData({ ...formData, manager_id: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
            ...
            .map((emp) => (<SelectItem key={emp.id} value={emp.id}>
                            {emp.first_name} {emp.last_name}
                          </SelectItem>))}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2">
                  ...
                  {canViewSalary ? (<Input type="number" value={formData.basic_salary} onChange={(e) => setFormData({ ...formData, basic_salary: e.target.value })}/>) : (<Input type="text" value="*" disabled className="cursor-not-allowed"/>)}
                </div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="shift">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.attendance_type_id || "_none_"} onValueChange={(value) => {
            ...
            setFormData({
                ...
                shift_type: selectedAttType?.is_shift_based ? "rotating" : "fixed"
                    ...
                    <SelectContent>
                      <SelectItem value="_none_">{language === "ar" ? "اختر" : "Select"}</SelectItem>
                      {attendanceTypes.map((at) => (<SelectItem key={at.id} value={at.id}>
            ...
            : "If attendance type is shift-based, it will be linked to shift session calendar"}
                ...
                <div className="space-y-2 flex items-center gap-3 pt-6">
                  <input type="checkbox" id="requires_attendance_signin" checked={formData.requires_attendance_signin} onChange={(e) => setFormData({ ...formData, requires_attendance_signin: e.target.checked })} className="h-4 w-4 rounded border-gray-300"/>
                ...
                return (<>
                          <div className="space-y-2">
                            ...
                            <Input type="time" value={hasFixedTimes ? selectedAttType.fixed_start_time?.slice(0, 5) || "" : formData.fixed_shift_start} onChange={(e) => !hasFixedTimes && setFormData({ ...formData, fixed_shift_start: e.target.value })} disabled={!!hasFixedTimes} className={hasFixedTimes ? "bg-muted" : ""}/>
                            {hasFixedTimes && (<p className="text-xs text-muted-foreground">
                                {language === "ar" ? "محدد من إعداد نوع الحضور" : "Set from attendance type setup"}
                              </p>)}
                          </div>
                          <div className="space-y-2">
                            <Label>{language === "ar" ? "نهاية الدوام" : "Work End"}</Label>
                            <Input type="time" value={hasFixedTimes ? selectedAttType.fixed_end_time?.slice(0, 5) || "" : formData.fixed_shift_end} onChange={(e) => !hasFixedTimes && setFormData({ ...formData, fixed_shift_end: e.target.value })} disabled={!!hasFixedTimes} className={hasFixedTimes ? "bg-muted" : ""}/>
                            {hasFixedTimes && (<p className="text-xs text-muted-foreground">
                                {language === "ar" ? "محدد من إعداد نوع الحضور" : "Set from attendance type setup"}
                              </p>)}
                          </div>
                          {selectedAttType && (selectedAttType.allow_late_minutes || selectedAttType.allow_early_exit_minutes) ? (<div className="md:col-span-2 p-3 bg-muted rounded-lg">
                              <p className="text-sm text-muted-foreground">
                                {language === "ar"
                ...
                {formData.attendance_type_id && attendanceTypes.find(at => at.id === formData.attendance_type_id)?.is_shift_based && (<div className="md:col-span-2 p-3 bg-muted rounded-lg">
                    <p className="text-sm text-muted-foreground">
                      {language === "ar"
            ...
            return (<TableRow key={vc.id} className={!isSelected ? "opacity-50" : ""}>
                              <TableCell>
                    ...
                    if (e.target.checked) {
                        ...
                        setEmployeeVacationBalances(employeeVacationBalances.filter(v => v.vacation_code_id !== vc.id));
                              ...
                              <TableCell>
                                <Input type="number" min="0" className="h-8 w-20" value={existingVac?.balance || 0} disabled={!isSelected} onChange={(e) => {
                    const newBalance = parseFloat(e.target.value) || 0;
                    if (existingVac) {
                        setEmployeeVacationBalances(employeeVacationBalances.map(v => v.vacation_code_id === vc.id ? { ...v, balance: newBalance } : v));
                    }
                    else {
                        setEmployeeVacationBalances([...employeeVacationBalances, {
                                employee_id: selectedEmployee?.id || "",
                              ...
                              <TableCell>
                                <Input type="number" min="0" className="h-8 w-20" value={existingVac?.used_days || 0} disabled={!isSelected} onChange={(e) => {
                    const newUsed = parseFloat(e.target.value) || 0;
                    if (existingVac) {
                        setEmployeeVacationBalances(employeeVacationBalances.map(v => v.vacation_code_id === vc.id ? { ...v, used_days: newUsed } : v));
                    }
                }}/>
                              </TableCell>
                              <TableCell>
            ...
            : `${selectedVacationTypes.length} vacation type(s) selected for year ${new Date().getFullYear()}`}
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.medical_insurance_plan_id || "_none_"} onValueChange={(value) => setFormData({ ...formData, medical_insurance_plan_id: value === "_none_" ? "" : value })}>
                    <SelectTrigger>
                      <SelectValue placeholder={language === "ar" ? "اختر" : "Select"}/>
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="_none_">{language === "ar" ? "اختر" : "Select"}</SelectItem>
                      {insurancePlans.map((plan) => (<SelectItem key={plan.id} value={plan.id}>
                          {plan.plan_name}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
      ...
      <Dialog open={documentDialogOpen} onOpenChange={setDocumentDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="border rounded-lg p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={documentFormData.document_type_id || "_none_"} onValueChange={(value) => setDocumentFormData({ ...documentFormData, document_type_id: value === "_none_" ? "" : value })}>
                  <SelectTrigger>
                    <SelectValue placeholder={language === "ar" ? "اختر النوع" : "Select Type"}/>
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="_none_">{language === "ar" ? "اختر" : "Select"}</SelectItem>
                    {documentTypes.map((dt) => (<SelectItem key={dt.id} value={dt.id}>
              ...
              <div className="space-y-2">
                ...
                <Input type="date" value={documentFormData.expiry_date} onChange={(e) => setDocumentFormData({ ...documentFormData, expiry_date: e.target.value })}/>
              </div>
            </div>
            <div className="space-y-2">
              <Label>{language === "ar" ? "ملاحظات" : "Notes"}</Label>
              <Input value={documentFormData.notes} onChange={(e) => setDocumentFormData({ ...documentFormData, notes: e.target.value })}/>
            </div>
            <Button onClick={() => documentInputRef.current?.click()} disabled={!documentFormData.document_type_id || uploadingDocument}>
          ...
          <div className="space-y-2">
            ...
            {employeeDocuments.length === 0 ? (<p className="text-sm text-muted-foreground py-4 text-center">
                {language === "ar" ? "لا توجد مستندات" : "No documents"}
              </p>) : (<div className="space-y-2">
                {employeeDocuments.map((doc) => (<div key={doc.id} className="flex items-center justify-between p-3 border rounded-lg">
      ...
      <Dialog open={payrollDialogOpen} onOpenChange={setPayrollDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          ...
          {payrollDialogLoading ? (<p className="text-center py-6 text-muted-foreground">{language === "ar" ? "جاري التحميل..." : "Loading..."}</p>) : payrollDialogRows.length === 0 ? (<p className="text-center py-6 text-muted-foreground">
              ...
              <TableBody>
                {payrollDialogRows.map((r) => (<TableRow key={r.id}>
      ...
      <Dialog open={loadUsersDialogOpen} onOpenChange={setLoadUsersDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {usersWithoutEmployee.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                ...
                <div className="border rounded-md max-h-[400px] overflow-y-auto">
                  {usersWithoutEmployee.map((user) => (<div key={user.user_id} className="flex items-center justify-between p-3 border-b last:border-b-0 hover:bg-muted/50">
                      <div className="flex items-center gap-3">
                        <input type="checkbox" checked={selectedUsersToAdd.includes(user.user_id)} onChange={() => toggleUserSelection(user.user_id)} className="h-4 w-4 rounded border-gray-300"/>
                        <div>
                          <p className="font-medium">{user.user_name}</p>
                          <p className="text-sm text-muted-foreground">{user.email}</p>
                        </div>
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => openAddFromUser(user)}>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddSelectedUsers} disabled={selectedUsersToAdd.length === 0}>
              <Plus className="h-4 w-4 mr-2"/>
              {language === "ar"
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            try {
                ...
                if (error)
                    throw error;
                setBusinessUnits((prev) => [...prev, data].sort((a, b) => a.unit_name.localeCompare(b.unit_name)));
                setFormData({ ...formData, working_business_unit_id: data.id });
                setAddBuOpen(false);
